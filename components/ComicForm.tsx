...
export default function ComicForm({ initialData, isEditing = false }: ComicFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isEditing && initialData?.id) {
        ...
        result = await supabase.from("comics").insert({
          title: form.title,
          slug: form.slug,
          description: form.description,
          thumbnail_url: form.thumbnail_url,
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-zinc-900 border border-zinc-800 rounded-lg p-8 max-w-2xl">
      {error && (
      ...
      <div className="space-y-6">
        ...
        <div>
          ...
          <input
            ...
            required
          />
          <p className="text-xs text-zinc-400 mt-1">Will be used in URLs like /comics/{form.slug}</p>
        </div>
      ...
      <div className="flex gap-4 mt-8">
        ...
        >
          {loading ? "Saving..." : isEditing ? "Update Comic" : "Create Comic"}
        </button>
        <button
          type="button"
          onClick={() => router.back()}
